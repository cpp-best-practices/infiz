...
void run_test(const std::string &input, RationalNumber expected, bool &success)
{
  ...
  if (result.getDenominator() != expected.getDenominator() || result.getNumerator() != expected.getNumerator()) {
    success = false;
    std::cout << "FAIL: ";
  } else {
    std::cout << "PASS: ";
  ...
  std::cout << "input: " << input << " expected: " << expected.getNumerator() << '/' << expected.getDenominator()
